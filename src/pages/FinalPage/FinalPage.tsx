...
export default function FinalPage() {
  ...
  const exportPDF = () => {
    ...
    const lines: string[] = [];
    if (general) {
      lines.push(`Lugar: ${general.lugar}`, `Fecha: ${general.fecha}`, `Receptor: ${general.receptor}`);
      if (general.otros) lines.push(`Otros: ${general.otros}`);
    }
    if (producer) {
      const p1 = `Productor: ${producer.nombre} ${producer.apellido}`;
      const p2 =
        [producer.documento && `Doc: ${producer.documento}`, producer.telefono && `Tel: ${producer.telefono}`, producer.email && `Email: ${producer.email}`]
        .filter(Boolean).join(' · ');
      lines.push(p1, p2);
    }
    doc.setFontSize(10);
    let y = 24;
    lines.filter(Boolean).forEach(l => { doc.text(l, 14, y); y += 5; });
  ...
  return (
    <div className="mt-3">
      ...
      <div className="mb-3 d-flex gap-2">
        <button className="btn btn-secondary" onClick={() => navigate('/productos')}>Volver a productos</button>
        <button className="btn btn-success" onClick={exportPDF} disabled={!general || !producer || products.length === 0}>Descargar PDF</button>
        <button className="btn btn-outline-secondary" onClick={() => window.print()} disabled={!general || !producer || products.length === 0}>Imprimir</button>
        <button className="btn btn-outline-danger" onClick={() => { clearAll(); navigate('/general'); }}>Reiniciar</button>
      </div>
