...
//     setProducer({ ...data, signature: signatureData });
//     navigate('/productos');
//   };
...
//             />
//           </div>
//           <button type='button' className='btn btn-sm btn-secondary mt-1' onClick={clearSignature}>Limpiar firma</button>
//         </div>
...
function ProducerPage() {
  ...
  // Cargar la firma existente si ya había
  useEffect(() => {
    if (producer?.signature && sigCanvas.current) {
      ...
      img.onload = () => {
        ...
        if (ctx) ctx.drawImage(img, 0, 0, sigCanvas.current!.getCanvas().width, sigCanvas.current!.getCanvas().height);
        setIsEmpty(false);
      };
    }
  }, [producer]);
  ...
  const clearSignature = () => {
    ...
    // Actualizar estado para que se borre la firma
    setProducer({
