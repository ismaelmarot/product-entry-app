...
function ProducerForm() {
  const { register, handleSubmit, reset, getValues } = useForm<Producer>();
  ...
  const clearSignature = () => {
    ...
    // actualizar solo la firma en el state
    setProducer((prev) => ({
      ...prev,
      signature: undefined,
    }));

    // actualizar solo la firma en react-hook-form
    reset(
      { ...getValues(), signature: '' },
